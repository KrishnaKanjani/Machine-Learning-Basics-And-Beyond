"""Decision tree for classification is a machine learning model that recursively splits the data into subsets based on the value of input features, creating a tree-like structure. At each node
...
"""The Decision_Tree_Classifier class is responsible for doing the following: (1) Handle the maximum depth, minimum samples and max features for splits (pre pruning); (2) Compute Gini 
...
class Decision_Tree_Classifier: 
   ...
   def grow_tree(self, x, y, depth=0):
      ...
      node = Node(value = leaf_value)   # create a node with predicted class value
   ...
   def calculate_leaf_node(self, y):
      """Calculate the predicted class for a leaf node (class with the highest count (mode))
         :param y : class label for current node
         :return : predicted class label for the leaf node
      """
      no_samples_per_class = np.bincount(y)   # no of samples for each class in the current node
      return np.argmax(no_samples_per_class)  
...
if __name__ == "__main__":
   ...
   tree = Decision_Tree_Classifier(max_depth=50, min_samples_split=3, max_features=4, criterion='gini')
   tree.fit(x_train, y_train)

   # Training the Sklearn decision tree model
   sklearn_tree = DecisionTreeClassifier(max_depth=50, min_samples_split=3, max_features=4, criterion='gini') 
